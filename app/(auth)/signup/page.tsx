...
function UserIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M5 19.5c1-3.6 3.6-5.5 7-5.5s6 1.9 7 5.5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/>
    </svg>
  );
}
...
export default function SignupPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const nextErrors = validate(name, email, password, confirm, agreed);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length > 0) return;

    setStatus('loading');
    // Wire this up to your real signup endpoint.
    setTimeout(() => {
      setStatus('success');
    }, 1200);
  };
        ...
        <div className={styles.brandContent}>
          ...
          <div className={styles.brandCopy}>
            <h1 className={styles.brandTitle}>
              Start building <span className={styles.brandAccent}>something real.</span>
            </h1>
            <p className={styles.brandSub}>
              Create an account to track projects, campaigns, and everything else we&rsquo;re building
              together.
            </p>
          </div>
        ...
        <div className={styles.formCard}>
          {status === 'success' ? (
            <div className={styles.successState}>
              ...
              <h2 className={styles.formTitle}>You&rsquo;re all set!</h2>
              <p className={styles.formSub}>Your account has been created.</p>
            </div>
          ) : (
            <>
              ...
              <form className={styles.form} onSubmit={handleSubmit} noValidate>
                <label className={styles.field}>
                  ...
                  <span className={`${styles.inputWrap} ${errors.name ? styles.inputError : ''}`}>
                ...
                <label className={styles.field}>
                  ...
                  <span className={`${styles.inputWrap} ${errors.confirm ? styles.inputError : ''}`}>
                    <span className={styles.inputIcon}><LockIcon /></span>
                    <input
                      type={showPassword ? 'text' : 'password'}
                      name="confirm"
                ...
                <label className={styles.checkboxRow}>
                  ...
                  />
                  <span>
                    I agree to the <a href="#" className={styles.inlineLink}>Terms</a> &amp;{' '}
